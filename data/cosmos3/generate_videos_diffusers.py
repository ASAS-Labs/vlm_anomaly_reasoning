"""generate_videos_diffusers.py - Cosmos3 text-to-video generation.

Generates 5s / 720p videos (no audio) for every structured JSON prompt under
data/cosmos3/video_gen_prompts/ using the Cosmos3 Diffusers pipeline. Writes 
the MP4s to data/datasets/generated_vids/ mirroring the prompt folder structure 
and names, and uploads each finished video to a Hugging Face dataset repo in the
background.

Setup (run once):

    ./setup_diffusers.sh   # system graphics libs (headless servers)
...
Run:
    ...
    uv run python data/cosmos3/generate_videos_diffusers.py
...
# Avoid the Xet hub backend. Must be set before importing huggingface_hub / diffusers.
