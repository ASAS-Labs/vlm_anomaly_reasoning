...
\caption{Context displacement in the six-field framework, on the stop-sign example. The object (1) and its nominal context (2), where reacting is correct, fan out into out-of-distribution contexts (3) through displacement operators (worn, carried, reprinted, reflected, projected). In the selected billboard context, the normal (4) and anomalous (5) ego actions define a scenario pair whose example prompts (6) seed video generation.}
\label{fig:framework}
\end{figure}
...
\begin{center}
\footnotesize
\begin{tabular}{|p{3.05cm}|c|c|}
\hline
\textbf{Stage-2 formulation} & \textbf{Acc.} & \parbox[c]{3.05cm}{\centering\textbf{Comparator acc.}\\\textbf{(correct expectation)}\vspace{1pt}} \\
\hline
